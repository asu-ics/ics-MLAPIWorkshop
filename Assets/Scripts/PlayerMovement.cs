...
{
    // Provides networked simple local player camera control and movement. 
    ...
    private void Start() //Make sure the local player's camera is the one that is active 
    {
        ...
        else//if not, disable the camera and audio listener
        {
    ...
    void Update()
    {
        if (IsLocalPlayer)//if the player is the correct local player, allow them to control it
        {
            Look();
            Move();
        }
    ...
    void Move()
    {
        ...
        //now add the gravity to the y velocity
        yVelocity -= gravity * Time.deltaTime;
        move.y = yVelocity;
        //and finally move
